...
def connect_wifi():
    wifi_list = ['GZPYP-WIFI', 'GZPYP-WIFI 2']
    for wifi in wifi_list:
        # 首次先查看是否当前已经连接了校园网
        try:
            ret = subprocess.check_output("netsh wlan show interfaces").decode('cp936')
        except UnicodeDecodeError:
            ret = subprocess.check_output("netsh wlan show interfaces").decode('utf-8')
        if wifi in ret:
            print('当前WIFI已经是校园网')
            return True
    for wifi in wifi_list:
        ...
        if wifi in ret:
            print('当前WIFI已经是校园网')
            return True
        ret = os.system(f'''cmd /c "netsh wlan connect name={wifi}"''')
        print(f'连接{wifi}, ret = {ret}')
        if ret == 0:
            while True:
                print('WIFI还没完全连接上')
                try:
...
class ConnectThread(QThread):
    ...
    def run(self) -> None:
        ...
        if not self.connected:
            print('连接')
            self.connected = self._first_page() and self._second_page()
        else:
            print('断开')
    ...
    def _first_page(self):
        ...
        if info['result'] == 'wait':  # success 为第一第二页面都登录了
    ...
    def logout(self):
        ...
        if not self.cip or not self.user_ip:  # 退出软件后是没有重新打开软件已经没有 cookie 可以退出
            self.user_ip = self.get_online_userinfo()['userIp']
            self.ss.cookies.clear()
    ...
    def get_verify(self):
        url = 'http://125.88.59.131:10001/common/image_code.jsp?time=' + str(int(round(time.time() * 1000)))
        res = self.ss.get(url)
        try:
